...
#DBLs Grab the time output in l118 for BOMD (FS)
#  (unfortunately, written twice per step in same format, hence the mod 2 appending...)
        elif line[1:10] == 'Time (fs)':
            time = float(line.split()[2]) * 41.3413745758
            timecount = timecount + 1
            if np.mod(timecount,2)==1:
              t.append(float(time))
#DBLe
...
#DBLs Grab the dipole output in l118 (in au) in case of BOMD
# (unfortunately, printed without whitespace between components if they happen to be negative...
# hence the crazy string manipulation))
        elif line[1:16] == 'Dipole        =': 
            tmp = lines[idx][0:16]+" "+lines[idx][16:31]+" "+lines[idx][31:46]+" "+lines[idx][46:]
            tmp = tmp.replace("D","e")
            dipole = tmp.split()
            muX.append(float(dipole[2])) 
            muY.append(float(dipole[3])) 
            muZ.append(float(dipole[4])) 
#DBLe
